# Engineering Docs Index

这个目录负责工程层文档，回答“系统怎么拆、按什么顺序落地、哪些边界不能破坏”。

## Files

- [architecture.md](D:/Code/VSCode_Project/StudyFlow/docs/engineering/architecture.md)
  - 逻辑架构、模块职责、核心实体、关键流程。
- [implementation-plan.md](D:/Code/VSCode_Project/StudyFlow/docs/engineering/implementation-plan.md)
  - MVP 分阶段实施计划、交付切片、反范围清单。
- [technology-selection.md](D:/Code/VSCode_Project/StudyFlow/docs/engineering/technology-selection.md)
  - 首版推荐技术栈、基础设施取舍、为什么现在这样选。

## Usage

- 当目录结构、模块边界或技术决策发生变化时，优先更新本目录文件。
- 如果这里只能通过引用产品规则才能成立，回到 `docs/product/` 先确认产品定义。
